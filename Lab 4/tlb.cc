...
static int FIFOPointer = 0;  // used to track the oldest entry 
...
int VpnToPhyPage(int vpn)
{
  ...
  for (int frameNum = 0; frameNum < NumPhysPages; frameNum++) {
      printf("IPT[%d] = %d, %d, %d, %d\n", frameNum, memoryTable[frameNum].pid, memoryTable[frameNum].vPage, 
	memoryTable[frameNum].lastUsed, memoryTable[frameNum].valid);
  }
...
void InsertToTLB(int vpn, int phyPage)
{
  ...
  for (int k = 0; k < TLBSize; k++) {
      printf("%i, %i, %i%c\t", machine->tlb[k].virtualPage, machine->tlb[k].physicalPage, 
	machine->tlb[k].valid, i==k?'*': ' '); 
      printf("\n"); 
  }
  
  // update the FIFOPointer to the new oldest entry after the new entry has been entered / oldest has been removed 
...
int lruAlgorithm(void)
{
  ...
  int phyPage = 0, oldestTime = 2147483647; // set earliestTime a max num in 2^31 - 1
  
  for (int i = 0; i < NumPhysPages; i++) {
      if(!memoryTable[i].valid) return i; 
      if (memoryTable[i].lastUsed < oldestTime) {
          oldestTime = memoryTable[i].lastUsed; 
          phyPage = i; 
      }
  }
